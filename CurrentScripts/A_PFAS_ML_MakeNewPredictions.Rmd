...
```{r configure_files}
OUTPUTFILE <- "new_inv_Dawson2023"
NEWPFASINFO <- "new_inv.csv"
INFO.SMILES.COL <- "QSAR_READY_SMILES"
NEWPFASOPERA <- "new_inv.csv"
OPERA.ID.COL <- "DTXSID"
NEWPFASENDO <- "EndoSubset-EPA_PFAS_DSSTox-Similarity_Patlewicz_113023.csv"
# Column with the CAS number for endogenous ligands:
ENDO.CAS.COL <- "ENDO.CASRN..EPA.PFAS."
NEWPFASTOXPRINT <- "toxprint_v2_vs_PFAStoPred.tsv"
```
  ...
  for (this.id in unique(pfastopred$DTXSID))
    if (this.id %in% endo.vals$DTXSID)
    {
      ...
      ifelse(as.numeric(endo.vals[endo.vals$DTXSID==this.id &
                endo.vals[,ENDO.CAS.COL] == this.cas,
...
# Replace any NA's if we have measurments (very few chemicals)
