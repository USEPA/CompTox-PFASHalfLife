...
#Pre-processing
PFASdata=read_xls("PFAS_Catalog/DSSTox_ComptoxDashboard_060921.xls")
#Output to Opera
...
##LOad name index:
paste("PFAS_Training_Set_Endo_Disc_HLH_unscaled_", readsuff,".RData", sep="")
...
Opera=Opera[,c(which(names(Opera)=="MoleculeID"),which(names(Opera)%in%Operacols))]
...
#########Check against training data
load(paste("PFAS_Training_Set_Endo_Disc_HLH_unscaled_", readsuff,".RData", sep=""))
pfasdsred1=data.frame(pfasds[,idcols], pfasdsred)
...
load(paste0("Kidney_Descriptors_in_Training_Set_", readsuff,".RData"))
...
load(paste("ClassificationModel_4Bin_EndoSimDisc_HLH_", readsuff, ".RData",sep="")) 
set.seed(seed)
...
#If its in the domain=1, if outside the doma
